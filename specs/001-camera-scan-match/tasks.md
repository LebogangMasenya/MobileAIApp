...
- [X] T042 Fix launch crash in `apps/mobile/src/app/_layout.tsx` — `ThemeProvider`, `DarkTheme`, and `DefaultTheme` were imported from `expo-router` (which does not export them), leaving `ThemeProvider` `undefined` and throwing `TypeError: Cannot read property 'displayName' of undefined` at render. Corrected the import to `@react-navigation/native` (the canonical Expo Router SDK 54 source, guaranteed present as an `expo-router` transitive dependency); `tsc --noEmit` reports zero errors in `_layout.tsx`
- [X] T043 Fix launch crash in `apps/mobile/src/components/app-tabs.tsx` — the tab triggers referenced `NativeTabs.Trigger.Label` / `NativeTabs.Trigger.Icon`, but in Expo Router SDK 54 `Label` and `Icon` are standalone named exports of `expo-router/unstable-native-tabs` (only `.TabBar` hangs off `NativeTabs.Trigger`), so those references were `undefined` and threw `TypeError: Cannot read property 'displayName' of undefined`. Switched to the `<Label>` / `<Icon>` exports, removed the unsupported `renderingMode` prop, and hardened the color-scheme lookup (`Colors[scheme ?? 'light']`, since `useColorScheme()` is never `'unspecified'` and null/undefined indexing crashed on `colors.*`). Applied the same color-scheme hardening to the web sibling `app-tabs.web.tsx`. `tsc --noEmit` reports zero errors in both `app-tabs.tsx` and `app-tabs.web.tsx`
